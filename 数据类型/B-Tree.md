# B-Tree特征  
1. 和二叉树一样，左小右大  
2. M阶的B树，其各节点数据(关键字)最多k-1个，最少1个，各节点子树个数最多为k个。其中，
M/2(向上取整)<K<M  
例子：一个3阶的B树，其节点子树最多为3个，各节点数据(关键字)最多为2个，最少为1。
3. 所有叶子节点位于同一层。

# 使用场景  
由于B树"矮矮胖胖"，不同于二叉树"高高瘦瘦",同时，由于磁盘io速度非常慢，为了减少磁盘的读写
次数，可用B树。
* Windows：HPFS 文件系统   
* Mac：HFS，HFS+ 文件系统  
...
# 参考链接  
* [重温数据结构：理解 B 树、B+ 树特点及使用场景](https://cloud.tencent.com/developer/article/1436534)

* [用Python实现B树](https://peefy.github.io/blog/2018/06/10/Python-BTree/)
